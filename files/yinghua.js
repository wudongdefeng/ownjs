    ...
    for (var i = 0; i < VideoListJson.length; i++) {
        ...
        for (var j = 0; j < VideoListJson[i][1].length; j++) {
            ...
            if (lists[2] == 'flv' || lists[2] == 'm3u8') {
                ...
                let lazy=`let jiexirescode_1=fetch(input,{headers:{'referer':'http://www.imomoe.in'}});let hx=jiexirescode_1.match(/合作.*/g);eval('/*'+hx[0]);let tm=jiexirescode_1.match(/"tm" value="(.*?)"/)[1];let csign=jiexirescode_1.match(/"get" value="(.*?)"/)[1];let v=jiexirescode_1.match(/"v": "(.*?)"/)[1];let cip=jiexirescode_1.match(/"cip": "(.*?)"/)[1];let cip_hex=jiexirescode_1.match(/"cip_hex": "(.*?)"/)[1];eval(getCryptoJS());eval(fetch('https://api.52wyb.com/js/md5.js?v=170726',{}));eval(fetch('https://api.52wyb.com/js/md5xp.js?v=170821',{}));let k2=md5(k3);function mark(pas){return md5("__fangdaolian__"+q(pas))+md5(q(pas)+"__fangdaolian__end")}let pad=jiexirescode_1.match(/vd .{1} "(.*?)"/)[1];let k=mark(vd+pad);var get=function(sess){var level=CryptoJS["MD5"](md5("6f9s2gb8d21w4x6v2j5f5o5b5d"+sess+"duoxieguanzhu")["substring"](16));var adjustedLevel=CryptoJS["enc"]["Utf8"]["parse"](level);var iv=CryptoJS["enc"]["Utf8"]["parse"](md5("elechipaomian"+sess["split"]("")["reverse"]()["join"]("")+"laotansuancaiweide")["substring"](16));var pixelSizeTargetMax=CryptoJS["AES"]["encrypt"](sess,adjustedLevel,{"iv":iv,"mode":CryptoJS["mode"]["CBC"],"padding":CryptoJS["pad"]["ZeroPadding"]});return"result:"+pixelSizeTargetMax["toString"]()};let cache=jiexirescode_1.match(/cache = (.*?);/)[1];let k4=encodeURIComponent(get(md5(md5(cache))));let jiexirescode_2=fetch('https://api.52wyb.com/action/playinfo?sver=1.4',{headers:{'referer':'https://api.52wyb.com','Accept':'application/json, text/javascript, */*;q=0.01','X-Requested-With':'XMLHttpRequest','Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},method:'POST',body:'k='+k+'&k2='+k2+'&k3='+k4+'&ep='+ep+'&cip='+cip+'&cip_hex='+cip_hex+'&csign='+csign+'&tm='+tm+'&v='+v+'&pt=auto&nip=null&from=http://www.imomoe.in&mode='});setError(jiexirescode_2);`;
